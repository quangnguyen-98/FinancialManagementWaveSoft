...
export default function PhieuThuScreen({ route}) {
    const dispatch = useDispatch();
    const navigation = useNavigation();
    const {idPT,ngayTraLai,tienLai,tienGoc,type} = route.params;
    ...
    const ngayTL =new Date(ngayTraLai);
    return (
        <View style={styles.thongTin}>
        <View style={styles.containerThongTinTongLai}>
            <View style={styles.menuTT}>
                <Text style={styles.textThongTin}>Ngày trả lãi: {`${ngayTL.getDate()}-${ngayTL.getMonth()+1}-${ngayTL.getFullYear()}`}</Text>
            </View>
            <View style={styles.menuTT}>
                <Text style={styles.textThongTin}>{type===3?'Tiền gốc':'Tiền lãi'}: {type===3?formatCurrency(tienGoc):formatCurrency(tienLai)}</Text>
            </View>
        </View>
    ...
    async function getCTHD(id) {
        try {
            ...
            let response = await fetch(apiLink + switchScreenReducers +'/chitiethopdongs/' + id + '?token=' + token);
    ...
    async function dongLai() {
        try {
            let token = await AsyncStorage.getItem('token');
            let response = await fetch(apiLink + switchScreenReducers +'/donglai?token=' + token, {
                method: 'PUT',
                headers: {
