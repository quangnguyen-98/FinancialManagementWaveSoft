   ...
   // const trangThaiDialog = useSelector(state => state.diaglogKhoaUserReducers);
            ...
            <FlatList
                ...
                renderItem={({item, index}) => (
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            dispatch({type:'SET_LOAIHD_DANGCHON',loaiHD:'dadong'});
    ...
    async function getAllHD(page) {
        try {
            ...
            let response = await fetch(apiLink + switchScreenReducers+'/hopdongs/dadong/' + page + '?usertype='+switchScreenReducers+'&token=' + token);
    ...
    async function timKiemDataHopDong(ten) {
        ...
        try {
            let token = await AsyncStorage.getItem('token');
            let response = await fetch(apiLink + switchScreenReducers+'/timhopdongs/dadong?ten=' + ten + '&token=' + token);
    ...
    async function xoaHopDong(idHopDong) {
        try {
            let token = await AsyncStorage.getItem('token');
            let response = await fetch(apiLink + switchScreenReducers+'/xoahopdongs?token=' + token, {
                method: 'PUT',
                headers: {
